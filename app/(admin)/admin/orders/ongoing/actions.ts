...
export async function moveToCompleted(orderId: string): Promise<ApiResponse> {
  try {
    ...
    try {
      await createAuditLog({
        ...
        description: `Moved order to completed (delivered)`,
      });
    } catch {
      console.error("Failed to create audit log for order:", orderId);
    }

    return { success: true, message: en.order_moved_to_completed };
  } catch (error: unknown) {
    if (error instanceof AuthError) throw error;
    return {
      success: false,
